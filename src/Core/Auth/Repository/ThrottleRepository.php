...
use \GameX\Core\Auth\Models\ThrottleModel;
use \Carbon\Carbon;
...
class ThrottleRepository implements ThrottleRepositoryInterface
{
    ...
    public function log($ipAddress = null, UserInterface $user = null)
    {
        ...
        if ($ipAddress !== null) {
            $ipAddressThrottle = new ThrottleModel();
        ...
        if ($user !== null) {
            $userThrottle = new ThrottleModel();
            $userThrottle->fill([
                'type' => 'user',
    ...
    protected function loadGlobalThrottles()
    {
        ...
        return ThrottleModel::where('type', 'global')
    ...
    protected function loadIpThrottles($ipAddress)
    {
        ...
        return ThrottleModel::where('type', 'ip')
            ->where('ip', $ipAddress)
            ->where('created_at', '>', $interval)
            ->get();
    }
    ...
    protected function loadUserThrottles(UserInterface $user)
    {
        ...
        return ThrottleModel::where('type', 'user')
